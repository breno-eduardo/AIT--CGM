...
# Lê a última sessão registrada na planilha (célula F2)
                ...
                for i in range(1, total_linhas + 1):
                    try:
                        ...
                        dados_coletados.append({
                            "PROCESSO": link_texto,
                            "SESSÃO MAIS RECENTE": nome_sessao,
    ...
    cabecalhos = [
            ...
            "PARTES/PROCURADORES", "SESSÃO MAIS RECENTE", "DATAS DAS DECISÕES",
            "DECISÕES", "ÚLTIMO ÓRGÃO TRAMITADO", "DATA ÚLTIMA TRAMITAÇÃO",
...
#ws.insert_rows(2, amount=num_linhas)
    ...
    df['DATA_FINAL_SESSAO'] = df['SESSÃO MAIS RECENTE'].apply(extrair_data_final_sessao)
#----------------------------------------------------
    # 2. Atualiza ou adiciona dados coletados
    for dado in dados_coletados:
        processo = str(dado["PROCESSO"]).strip()

        decisoes = dado["DECISÕES"].split("\n")
        datas = dado["DATAS DAS DECISÕES"].split("; ")

        texto_formatado = "\n".join([f"{len(decisoes) - i} - {d}" for i, d in enumerate(reversed(decisoes))])
        datas_formatadas = "\n".join([f"{len(datas) - i} - {data}" for i, data in enumerate(reversed(datas))])
