...
# Lê a última sessão registrada na planilha (célula F2)
    ...
    if os.path.exists(arquivo_excel):
        ...
        ultima_sessao_planilha = ws.cell(row=ws.max_row, column=6).value
         #Limpa da linha 2 em diante (mantém cabeçalhos e preserva colunas K e L)
        for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
            for idx, cell in enumerate(row):
                # Só limpa se NÃO for coluna K (11) ou L (12)
                if idx not in (10, 11):  # 0-indexado (coluna 11 é idx 10, coluna 12 é idx 11)
                    cell.value = None
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        processo = str(row[1].value).strip() if row[1].value else None  # Coluna B

        if processo:
            acao_sugerida = str(row[10].value).strip() if row[10].value else ""
            data_acao = str(row[11].value).strip() if row[11].value else ""

            if processo not in dados_manuais or (acao_sugerida or data_acao):
    ...
    # 3. Cria o DataFrame, ordena por sessão (mantendo todas as linhas de todos os processos)
    df = pd.DataFrame(dados_coletados)
    df = df.sort_values(by="SESSÃO")
    df_filtrado = df  # Sem remover duplicados de processo!
    ...
    for _, linha in df_filtrado.iterrows():
        ...
        # Aplica os dados manuais (colunas K e L) baseado no número do processo
        processo = linha["PROCESSO"]
        acao_sugerida, data_acao = dados_manuais.get(processo, ("", ""))

        ws.cell(row=linha_atual, column=11, value=acao_sugerida)  # Coluna K
        ws.cell(row=linha_atual, column=12, value=data_acao)      # Coluna L
